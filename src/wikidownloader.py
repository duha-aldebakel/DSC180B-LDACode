...
def get_random_wikipedia_article():
    ...
    while failed:
        ...
        try:
            ...
            '''
Duha note:
Sometimes the last line includes hints about a topic. 
For example, for https://en.wikipedia.org/wiki/Heather_Bagnall
s.find_all('p',class_='asbox-body')
Out[30]: [<p class="asbox-body">This article about a Maryland politician is a <a href="/wiki/Wikipedia:Stub" title="Wikipedia:Stub">stub</a>. You can help Wikipedia by <a class="external text" href="https://en.wikipedia.org/w/index.php?title=Heather_Bagnall&amp;action=edit">expanding it</a>.</p>]
It seems that these are tagged with class="asbox-body"
To avoid this, I explictly search for paragraphs with class_="", i.e. not set.                
            '''
...
if __name__ == '__main__':
    ...
    #Load old data
    try:
        with bz2.BZ2File('testdatapickle.bz2', 'rb') as f:  #Use datacompression BZ2
            data = pickle.load(f)
    except:
        data=([],[])
       

    (articles, articlenames) = get_random_wikipedia_articles(1000)

    newdata=(articles, articlenames)
